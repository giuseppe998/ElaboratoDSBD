    ...
    for metric in metrics:
        ...
        def Seasonality():  
            ...
            for i in range (0,9): #provo i periodi 10 volte levando il 10% ogni iterazione
                ...
                acf_season = acf(result.seasonal,nlags=df.shape[0])
                plt.figure(figsize=(24,10))
                #add axes labels and a title
                plt.ylabel('Values', fontsize=14)
                plt.xlabel('Time', fontsize=14)
                plt.title('Graph', fontsize=16)
                plt.plot(acf_season,"--",label = 'Seasonality')
                plt.show()
                ...
                for j in range(0,len(acf_error) -1): #scorro tutti i campioni di acf e verifico se si ha un comportamento statistico simil white noise
                    if not (acf_error[0] == 1 and abs(acf_error[j]) <= 0.01):
                     ...
                     return {'Trend' : result.trend, 'Seasonality' : result.seasonal,'Period': int(period)}
            else:     
                result = seasonal_decompose(df.get('value'), model='add', period= int(minPeriod))
                return {'Trend' : result.trend, 'Seasonality' : result.seasonal,'Period': int(minPeriod)}
        ...
        def pred():
            ...
            fcast = ARfit.predict(start=len(data), end=len(data)+10, dynamic=False).rename('Forecast')
        ...
        plt.plot(seasonality['Seasonality'],"--",label = 'Seasonality', color = "blue")
        plt.legend("value")
        plt.show()
